...
#| label: fig-500-sim
#| echo: false
#| fig-cap: "Boxplots of the results for n = 500 simulations."
#| fig-subcap: 
#|     - "Boxplots of the differences between the complete data and incomplete data estimates for the odds ratio of the race effect."
#|     - "Boxplots of the odds ratios for the race effect"
#| layout-nrow: 2
...
The bias is corrected for when we use multiple imputation with an appropriate number of imputations [@grahamHowManyImputations2007; @harelInferencesMissingInformation2007]. The multiple imputation analysis produces unbiased results that will measure the true race effect on average when the missing data are generated under a MAR mechanism or when we include some model for the missingness mechanism in the imputation model to address the MNAR assumption [@bartlettAsymptoticallyUnbiasedEstimation2015] [**more citations**].

In our simulations, we assume that the missing data mechanism is MAR and ignorable, so we omit any modeling of the mechanism in our imputations. The estimates made with MI under the MAR generated data are unbiased and closely resemble the complete data estimates demonstrating that the MI procedure is able to recover most of the information lost under CCA. While not addressing the missingness model is appropriate for the MAR generated data, it fails to overcome the bias induced by the MNAR generated data resulting in only slightly mitigated biases in the MI estimates with MNAR data. Tools have been developed to address the MNAR assumption by creating models for the missingness in the imputation mechanism [**citation**]. Those tools are beyond the scope of this paper.

```{r}
#| label: fig-1000-sim
#| echo: false
#| fig-cap: "Boxplots of the results for n = 1000 simulations."
#| fig-subcap: 
#|     - "Boxplots of the differences between the complete data and incomplete data estimates for the odds ratio of the race effect."
#|     - "Boxplots of the odds ratios for the race effect"
#| layout-nrow: 2
...
#| label: fig-2500-sim
#| echo: false
#| fig-cap: "Boxplots of the results for n = 2500 simulations."
#| fig-subcap: 
#|     - "Boxplots of the differences between the complete data and incomplete data estimates for the odds ratio of the race effect."
#|     - "Boxplots of the odds ratios for the race effect"
#| layout-nrow: 2
...
I also did a simulation with 105 iterations, a sample size of 25,000, and $M = 3$ imputations, since that still gives roughly 95% relative efficiency but reduces the computational cost significantly. Results are shown in @fig-25000-sim. From these simulations, we further establish the patterns seen with the smaller sample sizes. Roughly 5-10% of cases are incomplete. The distribution of the differences is narrower than for the smaller samples and the median differences are still clearly different from zero and roughly equivalent to the median differences under the $n = 2,500$ simulations as seen in @fig-2500-sim. As this simulation is 10x larger than the 2500 sample size simulation, we can likely expect these patterns to continue for even larger sample sizes. Large data is therefore not a cure-all for missing data and still must be given special attention.

```{r}
#| label: fig-25000-sim
#| echo: false
#| fig-cap: "Boxplots of the results for n = 25000 simulations."
#| fig-subcap: 
#|     - "Boxplots of the differences between the complete data and incomplete data estimates for the odds ratio of the race effect."
#|     - "Boxplots of the odds ratios for the race effect"
#| layout-nrow: 2
