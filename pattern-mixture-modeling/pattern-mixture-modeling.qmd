...
#| tbl-cap: Summary statistics on sentence length (months) and incarceration.
#| 
df |>
    # filter(JP_MIN_MON > 0) |>
    group_by(YEAR) |>
...
#| fig-cap: Density plots for A) Sentence length (mon.) by year. B) Log sentence lengths (mon.) plus one month by year.

p1 <- df |>
    # filter(JP_MIN_MON > 0) |>
    ggplot(aes(YEAR, JP_MIN_MON)) +
...
p2 <- df |>
    # filter(JP_MIN_MON > 0) |>
    ggplot(aes(YEAR, log1p(JP_MIN_MON))) +
    geom_violin(draw_quantiles = c(0.025, 0.25, 0.5, 0.75, 0.975)) +
        facet_wrap(YEAR~., scales = "free", nrow = 5) +
    coord_flip() +
    labs(x = "") +
...
#| fig-cap: A) Sentence length (mon.) by most serious crime type. B) Log of sentence length plus one month by most serious crime type.
#| fig-height: 4

p1 <- ggplot(df, aes(CRIME, JP_MIN_MON)) +
    geom_violin(draw_quantiles = c(0.025, 0.25, 0.5, 0.75, 0.975)) +
    geom_point(size = 0.5)

p2 <- ggplot(df, aes(CRIME, log(JP_MIN_MON))) +
    geom_violin(draw_quantiles = c(0.025, 0.25, 0.5, 0.75, 0.975)) +
    geom_point(size = 0.5)
...
#| fig-cap: A) Sentence length by offender race. B) Log of sentence length (plus one month)
#| fig-height: 4
p1 <- ggplot(df, aes(OFF_RACE, JP_MIN_MON)) +
    geom_violin(draw_quantiles = c(0.025, 0.25, 0.5, 0.75, 0.975)) +
    geom_point(size = 0.5)

p2 <- ggplot(df, aes(OFF_RACE, log(JP_MIN_MON))) +
    geom_violin(draw_quantiles = c(0.025, 0.25, 0.5, 0.75, 0.975)) +
    geom_point(size = 0.5)
...
#| layout-ncol: 2
#| fig-cap: QQ plots of the sentence length (months)
#| fig-subcap: 
...
I'll prepare the data for analysis next. First, I'll center and scale the numeric predictors which are Offense Gravity Score (OGS), defendant age, and the square of each. Then I'll use MICE to multiply impute the incomplete variables with random forest. For now I'll use $M = 10$ since less than 5% of all observations are missing (mainly in defendant race).
...
saveRDS(imps, "mice-rf-subsample.rds")
```
...
imps <- readRDS("mice-rf-subsample.rds")
```
...
fit_incar <- with(imps, glm(INCAR ~ DOSAGE + DOSAGEQ + SEX*OFF_RACE + OGS + OGSQ +
                 ...
                 x = FALSE, y = FALSE,
                 model = FALSE))
saveRDS(fit_incar, "fit-glm-logit-incar.rds")
```
...
fit_incar <- readRDS("fit-glm-logit-incar.rds")
...
saveRDS(fit_incar_mm, "fit-glmm-logit-incar.rds")
```

```{r}
#| label: tbl-glmm-sum
#| tbl-cap: Summary statistics for the fixed effects from the logistic regression mixed model.
fit_incar_mm <- readRDS("fit-glmm-logit-incar.rds")
...
saveRDS(fit_brm, file = "fit-brms-hurdle-lognormal-mi.rds")
```
...
fit_brm <- readRDS("fit-brms-hurdle-lognormal-mi.rds")
...
#|   - Density for log of sentence length plus one month
...
fit_brm2 <- brm_multiple(bf1,
               ...
               # file = "brms_fit/brms-hurlde-si.fit",
               # file_refit = "on_change",
               save_model = "brms-hurdle-lognormal-si.stan")

saveRDS(fit_brm2, "fit-brms-hurdle-lognormal-si.rds")
```
...
fit_brm2 <- readRDS("fit-brms-hurdle-lognormal-si.rds")
...
```{r}
#| label: fig-cond-eff-brm2-race-sex
#| include: false
#| fig-cap: Posterior estimates of conditional effects for race and sex on log sentence length. Full model with linear predictor for the logistic regression of the zero part of the hurdle regression.
...
#| fig-subcap: 
#|   - PPC Density Sentence Length (months)
...
bprior3$class
bprior3 <- prior(normal(0, 10), class = "b") +
    prior(student_t(3, 0, 2.5), class = "Intercept")
    prior(inv_gamma(2.001, 1.001), class = "sd", group = "COUNTY", lb = 0) +
    prior(inv_gamma(2.001, 1.001), class = "sd", group = "YEAR", lb = 0) +
    prior(logistic(0, 1), class = "Intercept", dpar = "hu") +
...
fit_brm3 <- brm(bf3,
               ...
               prior = bprior3,
               init = "0",
               chains = 2, cores = 15, iter = 5000, refresh = 100,
               # file = "brms_fit/brms-hurlde-si.fit",
               # file_refit = "on_change",
               control = list(max_treedepth = 12, adapt_delta = 0.85),
               save_model = "brms-hurdle-si-pois.stan")

saveRDS(fit_brm3, "fit-brms-hurdle-pois-si.rds")
```

```{r}
#| label: tbl-sum-brms-hurdle-poisson
#| include: false
#| 

fit_brm3 <- readRDS("fit-brms-hurdle-pois-si.rds")
...
```{r}
#| label: fig-cond-eff-brm3-nonzero
#| include: false
#| layout-ncol: 3
#| layout-nrow: 4
#| fig-cap: Posterior estimates of the conditional effects for the Poisson glm for the non-zero-part of the hurdle Poisson model.
#| fig-subcap: 
#|   - Age
#|   - Age Squared
#|   - Sex
...
```{r}
#| label: fig-cond-eff-brm3
#| include: false
#| layout-ncol: 3
#| layout-nrow: 4
#| fig-cap: Posterior estimates of the conditional effects for the logistic regression on the zero-part of the hurdle Poisson model.
#| fig-subcap:
#|   - Age
#|   - Age Squared
#|   - Sex
...
#|   - Race by Sex
#|   - OGS by Prev. Rec.
plot(conditional_effects(fit_brm3, dpar = "hu"), ask = FALSE)
```
...
#| include: false
#| layout-ncol: 3
#| fig-cap: Posterior Predictive Checks for full Poisson Hurdle model.
#| fig-subcap:
#|   - PPC Density Sentence Length (months)
#|   - PPC Density Log of Sentence Length plus 1
#|   - PPC Scatter of average error by age
...
fit_lnh <- mixed_model(JP_MIN ~ DOSAGE + DOSAGEQ + SEX*OFF_RACE + OGS + OGSQ +
                    ...
                    random = ~ 1 |COUNTY,
                    zi_fixed = ~ SEX * OFF_RACE + RECMIN + OGS + OGSQ + PRVREC + CRIME + TRIAL,
                    zi_random = ~ 1 | COUNTY)
...
fit_rf <- ranger::ranger(OFF_RACE ~ DOSAGE + DOSAGEQ + SEX + TRIAL + RECMIN + PRVREC + OGS + OGSQ + CRIME + YEAR + COUNTY + JP_MIN_MON, 
                         data = c_dat_s,
                         probability = TRUE)
...
plab <- sample_rf_pd(c_dat_s, fit_rf, scale = c(1.25, 1, 1, 1))
table(plab)
```
...
modify_race_imps <- function(imps, scale = c(1.5, 1, 1, 1)) {
    imp_list <- complete(imps, "all", include = TRUE)
